 ...
 * This class handles the user interface and logic for adding, deleting, and updating dishes in the menu.
 */
public class UpdateMenuController {
    ...
    private ObservableList<String> categories = FXCollections.observableArrayList("Salad", "Main Course", "Dessert", "Drink");
    ...
    public static class DishDisplay {
        ...
        public String getDishName() { return dish != null ? dish.getDishName() : ""; }
        public String getOptionValue() { return option != null ? option.getOptionValue() : "None"; }
        public String getSize() { return price != null ? price.getSize() : "None"; }
     ...
     * Sets up the table columns, choice boxes, and loads the initial dish data.
     *
     * @throws IOException if there's an error loading the dishes
     */
    public void initialize() throws IOException {
        instance = this;
        if (nameColumn != null) nameColumn.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getDishName()));
        if (optionColumn != null) optionColumn.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getOptionValue()));
        if (sizeColumn != null) sizeColumn.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getSize()));
        if (priceColumn != null) priceColumn.setCellValueFactory(cellData -> new SimpleIntegerProperty(cellData.getValue().getPrice()).asObject());
        if (categoryColumn != null) categoryColumn.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getCategoryName()));
        if (txtTable != null) txtTable.setItems(dishList);
    ...
    /**
     * Loads the dishes for the current restaurant.
     * Sends a message to the server to retrieve the dish data.
     *
    ...
    /**
     * Sets the restaurant number and name for the current session.
     * This method is used to initialize or update the restaurant information in the controller.
     *
    ...
    /**
     * Requests the restaurant number associated with the certified employee.
     * Sends a message to the server to retrieve the restaurant number.
     *
    ...
    /**
     * Requests the restaurant name associated with the certified employee.
     * Sends a message to the server to retrieve the restaurant name.
     *
     * @param user The certified employee user
     * @throws IOException if there's an error communicating with the server
     */
    ...
    /**
     * Checks if a dish with the given name and size already exists in the restaurant's menu.
     * Sends a message to the server to perform the check.
     *
    ...
    public void isDishAlreadyExists(String dishName, String size) throws IOException {
        // Send a message to the server to check if a dish with the same name and size already exists
    ...
    /**
     * Handles the action of adding a new dish to the menu.
     * Validates input, checks for existing dishes, and sends the new dish data to the server.
     *
    ...
    void addDish(ActionEvent event) throws IOException {
        ...
        if (restaurantNumber <= 0) {
         ...
         // Check if a dish with the same name and size already exists
            isDishAlreadyExists(dishName, size); // The addDish logic will be triggered from handleServerResponse if dishExists is false.
    }
 
    
    /**
     * Handles the action of deleting a dish from the menu.
     * Sends a delete request to the server for the selected dish.
     *
    ...
    /**
     * Handles the action of updating the price of a selected dish.
     * Validates the selected dish and new price input, then sends an update request to the server.
     *
        ...
        case CheckDishExists:
            ...
            if (dishExists) {
                ...
                try {
                    ...
                    Message addDishMessage = new Message(new Object[]{newDish, newPrice, newOption}, Commands.AddDish);
    ...
    /**
     * Handles the selection of a dish option type.
     * Controls the visibility of the dish option input field based on the selected option.
     *
     * @param selectedOption The option selected by the user ("None", "INGREDIENT", or "COOKING_LEVEL")
     */
    ...
    /**
     * Retrieves the category ID based on the selected category name.
     * Maps category names to their corresponding numeric IDs.
     *
     * @return The numeric ID of the selected category, or 0 if no category is selected
     */
    ...
    /**
     * Retrieves the OptionType enum based on the selected option string.
     * Maps option strings to their corresponding OptionType enum values.
     *
     * @return The OptionType enum value corresponding to the selected option, or null if no option is selected
     */
     ...
     * Used for showing information and error messages to the user.
     *
